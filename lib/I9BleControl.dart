...
typedef GetTerminalInfoCallback = void Function(
    I9TerminalInfo? terminalInfo, int status);
//获取密钥信息回调
typedef GetSecretKeyInfoCallback = void Function(
    String? keyInfo, String? kcv, String? ksn, int status);
//PIN加密回调
typedef TerminalEncryptionPINCallback = void Function(
    String? pinData, String? DUKPTKey, int status);
//请求数据加密回调
...
//返回卡片信息
typedef ReadCardCallback = void Function(I9CardInfo? cardInfo, int status);
//读卡错误回调
...
class BleControl {
  ...
  //i9服务id
  static String _i9Service = '49535343-FE7D-4AE5-8FA9-9FAFD205E455';
  //i9通知特征
  static String _i9NotiChar = '49535343-1E4D-4BD9-BA61-23C647249616';
  //i9写数据特征
  static String _i9WriteChar = '49535343-8841-43F4-A8D4-ECBE34729BB3';

  var _notifySubscription;
  //BluetoothCharacteristic _m6pNotiChar1;
  late BluetoothCharacteristic _i9WriteChar1;
  //蓝牙发送包长
  ...
  _receiveData1(String str) {
    ...
    if (resCode1 == 0x09 && resCode2 == 0x1b) {
      if (resultBuf[3] == 0x01) {
        I9TerminalInfo terminalInfo = I9TerminalInfo();
        if (res != 0) {
          ...
          for (TLV tlv in arr) {
            //sn
            if (tlv.tag == '1F80') {
              terminalInfo.sn = _ascToHex(tlv.value!);
            }
            //mac
            if (tlv.tag == '1F81') {
            ...
            //App版本日期
            if (tlv.tag == '1F85') {
              terminalInfo.softVersionDate = _ascToHex(tlv.value!);
            }
            //Kernel版本
            if (tlv.tag == '1F86') {
          ...
          _queryTMS(
              terminalInfo.sn.toString(),
              terminalInfo.softVersion.toString(),
        ...
        if (res != 0) {
          ...
          if ((resultBuf[11] & 1) != 0) {
            ...
            tLen = tLen + 1;
            String data1 = str.substring(tLen * 2, tLen * 2 + tsnLen * 2);
            terminalInfo.tsn = data1;
            tLen = tLen + tsnLen;
          }
          //版本号
          if ((resultBuf[11] & 0x02) != 0) {
            int sofeversionLen = resultBuf[tLen];
            print(sofeversionLen);
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + sofeversionLen * 2);
            terminalInfo.softVersion = _ascToHex(data1);
            tLen = tLen + sofeversionLen;
          }
          //蓝牙名称
          if ((resultBuf[11] & 0x04) != 0) {
            int bluetoothNameLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + bluetoothNameLen * 2);
            terminalInfo.bluetoothName = _ascToHex(data1);
            tLen = tLen + bluetoothNameLen;
          }
          //蓝牙版本
          if ((resultBuf[11] & 0x08) != 0) {
            int bluetoothVersionLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + bluetoothVersionLen * 2);
            terminalInfo.bluetoothVersion = _ascToHex(data1);
            tLen = tLen + bluetoothVersionLen;
          }
          //蓝牙mac地址
          if ((resultBuf[11] & 0x10) != 0) {
            int bluetoothMACLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + bluetoothMACLen * 2);
            terminalInfo.bluetoothMAC = _ascToHex(data1);
            tLen = tLen + bluetoothMACLen;
          }
          //版本日期
          if ((resultBuf[11] & 0x20) != 0) {
            int softVersionDateLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + softVersionDateLen * 2);
            terminalInfo.softVersionDate = _ascToHex(data1);

            tLen = tLen + softVersionDateLen;
          }
          //psam卡号
          if ((resultBuf[11] & 0x40) != 0) {
            int psamNoLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 = str.substring(tLen * 2, tLen * 2 + psamNoLen * 2);
            terminalInfo.psamNo = data1;

            tLen = tLen + psamNoLen;
          }
          //下面还有21号文的，海外的不解析
          //密钥标志
          if ((resultBuf[11] & 0x80) != 0) {
            int protocolTypeLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + protocolTypeLen * 2);

            terminalInfo.protocolType = data1;
            tLen = tLen + protocolTypeLen;
          }
          //SN
          if ((resultBuf[12] & 0x01) != 0) {
            int snLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 = str.substring(tLen * 2, tLen * 2 + snLen * 2);

            terminalInfo.sn = data1;
            String? str1 = terminalInfo.sn;
            if (str1!.length >= 6) {
              if (str1.substring(0, 6) != "000030") {
                terminalInfo.sn = _ascToHex(str1);
              }
            }
            tLen = tLen + snLen;
          }
          //kernel版本
          if ((resultBuf[12] & 0x02) != 0) {
            int kernelVersionLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + kernelVersionLen * 2);

            terminalInfo.kernelVersion = _ascToHex(data1);
            tLen = tLen + kernelVersionLen;
          }
          //硬件版本
          if ((resultBuf[12] & 0x04) != 0) {
            int hardwareVersionLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + hardwareVersionLen * 2);

            terminalInfo.hardwareVersion = _ascToHex(data1);
            tLen = tLen + hardwareVersionLen;
          }
          //固件版本
          if ((resultBuf[12] & 0x08) != 0) {
            int firmwareVersionLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + firmwareVersionLen * 2);

            terminalInfo.firmwareVersion = _ascToHex(data1);
            tLen = tLen + firmwareVersionLen;
          }
          //CPUSN
          if ((resultBuf[12] & 0x10) != 0) {
            int cpuSNLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 = str.substring(tLen * 2, tLen * 2 + cpuSNLen * 2);

            terminalInfo.cpuSN = data1;
            tLen = tLen + cpuSNLen;
          }
          //customerSN
          if ((resultBuf[12] & 0x20) != 0) {
            int customerSNLen = resultBuf[tLen];
            tLen = tLen + 1;
            String data1 =
                str.substring(tLen * 2, tLen * 2 + customerSNLen * 2);
            terminalInfo.customerSN = data1;
    ...
    if (resCode1 == 0x02 && resCode2 == 0xa0) {
      ...
      if (res == 0x00) {
        ...
        I9CardInfo cardInfo = I9CardInfo();
        if (str.length > 18) {
          int failLength = resultBuf[7] * 256 + resultBuf[8];
    ...
    if (resCode1 == 0x02 && resCode2 == 0x20) {
      if (res != 0) {
        ...
        String pinEncry = str.substring(18, 18 + pinEncryLen * 2);
        _pinEntryCallback1(pinEncry, res);
      }
    }
    //pin加密
    if (resCode1 == 0x02 && resCode2 == 0xA2) {
      if (res != 0) {
        _terminalEncryptionPINCallback('', '', res);
      } else {
        // int pinEncryLen = resultBuf[7] * 256 + resultBuf[8];
        // String pinEncry = str.substring(18, 18 + pinEncryLen * 2);
        List<TLV> arr = _getTLVArr(dataStr);
        ...
        _terminalEncryptionPINCallback(pin, ksn, res);
      }
    }
  ...
  _analyzI9(String str) {
    ...
    for (TLV tlv in arr) {
      ...
      //卡类型
      if (tlv.tag == '1F41') {
        cardInfo.cardType = _hexToInt(tlv.value!.substring(0, 2)) & 0x07;
  ...
  _sendComd(String commandStr) async {
    ...
    for (int i = 0; i < arr.length ~/ _bluetoothNumber; i++) {
      ...
      await _i9WriteChar1.write(arr1);
    }
  ...
  //搜索蓝牙
  Stream<I9BleDevice> startScan() async* {
    await _flutterBlue.stopScan();
    _blueList = [];
    yield* _flutterBlue.scan().transform(
        StreamTransformer<ScanResult, I9BleDevice>.fromHandlers(
      ...
      if (device.name.length > 0) {
        ...
        if (!isExist) {
          ...
          I9BleDevice scanResult1 = I9BleDevice();
  ...
  //连接蓝牙
  Future<bool> connectDevice(I9BleDevice device) async {
    await _flutterBlue.stopScan();
    ...
    for (BluetoothDevice bleDevice in _blueList) {
      if (bleDevice.id.toString() == device.UUID) {
        ...
        services.forEach((service) {
          if (service.uuid.toString().toUpperCase() == _i9Service) {
            List<BluetoothCharacteristic> characteristics =
                service.characteristics;
            characteristics.forEach((characteristic) {
              if (characteristic.uuid.toString().toUpperCase() == _i9NotiChar) {
                _receiveData(characteristic);
                isConnect1 = true;
              }
              if (characteristic.uuid.toString().toUpperCase() ==
                  _i9WriteChar) {
                _i9WriteChar1 = characteristic;
                isConnect2 = true;
                print(isConnect2);
              }
            });
  ...
  //获取设备类型
  getTerminalInfo(
      void Function(I9TerminalInfo? terminalInfo, int status)
 ...
 */
  ...
  //更新AID
  downloadAIDParameters(I9AIDParameters aidParameters,
  ...
  //更新RID
  downloadPublicKey(I9CAPublicKey caPublicKey,
 ...
 */
  requestDataDecrypt(
      ...
      int operationMode,
      String ivData,
      void Function(String? decryptData, String? ksn, int status)
 ...
 */
  ...
  late TerminalEncryptionPINCallback _terminalEncryptionPINCallback;
  //PIN加密
  terminalEncryptionPIN(
      String controlFlag,
      String pin,
      String keyType,
      void Function(String? pinData, String? DUKPTKey, int status)
          terminalEncryptionPINCallback) {
    _terminalEncryptionPINCallback = terminalEncryptionPINCallback;
    pin = _hexToAsc(pin);
    String data = '1F0A' + _intToHex(pin.length ~/ 2) + pin;
    data = '1F0406' + keyType + data;
    int len = data.length ~/ 2;
    String str =
        '0002A210' + _intToHex(len ~/ 256) + _intToHex(len % 256) + data;
    _sendComd(str);
  }
  ...
  startEmvProcess(
      int timeout,
      I9TradeData tradeData,
      void Function() waitingcardCallback,
      void Function() icCardInsertionCallback,
      void Function() nfcCardDetectionCallback,
      void Function(I9CardInfo? cardInfo, int status) readCardCallback,
    ...
    //控制标志
    String str2 = _getTLVStr('1F02', tradeData.sign);
    str = str + str2;
    ...
    String str3 = _getTLVStr('1F03', time);
    str = str + str3;
    len = len + str3.length ~/ 2;
    //加密算法
    if (tradeData.encryptionAlgorithm != null) {
      String str1 = _getTLVStr('1F04', tradeData.encryptionAlgorithm!);
      str = str + str1;
      len = len + str1.length ~/ 2;
    }
    //刷卡标题
    if (tradeData.swipeTitle != null) {
      String str1 =
          _getTLVStr('1F05', _arrToStr(gbk.encode(tradeData.swipeTitle!)));
      str = str + str1;
      len = len + str1.length ~/ 2;
    }
    //密码输入标题
    if (tradeData.pinTitle != null) {
      String str1 =
          _getTLVStr('1F06', _arrToStr(gbk.encode(tradeData.pinTitle!)));
      str = str + str1;
      len = len + str1.length ~/ 2;
    }
    //随机数
    ...
    //附加数据
    if (tradeData.extraData != null) {
      String str1 = _getTLVStr('1F0A', _hexToAsc(tradeData.extraData!));
      str = str + str1;
      len = len + str1.length ~/ 2;
    }
    //回显数据
  ...
  //开始NFC/IC
  powerOnAPDU(int type, int timeout, void Function() waitingcardCallback,
